...
export async function addItemToCart(cartId: string, productId: string, _price: number, quantity: number = 1) {
  try {
    const q = query(collection(db, 'cart_items'), where('cart_id', '==', cartId), where('product_id', '==', productId));
    const snapshot = await getDocs(q);
